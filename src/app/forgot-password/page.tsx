      ...
      <div className="w-full max-w-md">
        ...
        {sent ? (
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 text-center">
            ...
            <Link href="/login" className="text-[#F77B0F] font-medium text-sm">Back to Sign In</Link>
          </div>
        ) : (
          <form onSubmit={onSubmit} className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-8 space-y-5">
            <div><label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Email</label><input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-[#F77B0F] outline-none" placeholder="you@example.com" /></div>
            <button type="submit" disabled={submitting} className="w-full py-3 bg-[#F77B0F] text-white font-semibold rounded-lg hover:bg-[#e06a0d] disabled:opacity-50">{submitting ? "Sending..." : "Send Reset Link"}</button>
          </form>
        )}
        {!sent && <p className="text-center text-sm text-gray-500 mt-6"><Link href="/login" className="text-[#F77B0F] font-medium">Back to Sign In</Link></p>}
      </div>
    </div>
  );
}

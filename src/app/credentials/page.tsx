            ...
            >
              ...
              {t.count > 0 && (
                <span className={cn(
                  ...
                  tab === t.key
                    ? "bg-[#192C67]/10 text-[#192C67] dark:bg-[#192C67]/20 dark:text-white/70"
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-zinc-300 dark:border-zinc-700 py-16 text-center">
          <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#192C67]/10 dark:bg-[#192C67]/20">
            <svg className="h-8 w-8 text-[#192C67] dark:text-white/70" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                        ...
                        {cert.owner && !cert.owner.isMe && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-[#192C67]/10 dark:bg-[#192C67]/20 px-2 py-0.5 text-[10px] font-semibold text-[#192C67] dark:text-white/70">
          ...
          <div className="space-y-2">
            {data.requests.map((r: any) => (
              <div key={r.id} className="flex items-center gap-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 py-3">
                ...
                {r.documentUrl && (
                  <a href={r.documentUrl} target="_blank" rel="noopener noreferrer" className="shrink-0 text-xs font-semibold text-[#192C67] dark:text-white/70 hover:underline">
                    View
                  </a>
                )}
              </div>
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      selectedMemberId === m.profileId
                        ? "border-[#192C67] bg-[#192C67]/5 dark:border-[#F77B0F]/50 dark:bg-[#192C67]/10"
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-zinc-900 dark:text-zinc-100 truncate">
                        ...
                        {m.isMe && <span className="ml-1.5 text-[10px] font-bold text-[#192C67] dark:text-white/70">(you)</span>}
                      </p>
                      <p className="text-[11px] text-zinc-500 dark:text-zinc-400">{m.role}{m.tier ? ` · ${m.tier}` : ""}</p>
                    </div>
                    {selectedMemberId === m.profileId && (
                      <svg className="h-4 w-4 shrink-0 text-[#192C67] dark:text-white/70" fill="currentColor" viewBox="0 0 20 20">
              ...
              {detail.documentUrl && (
                <div className="border-t border-zinc-200 dark:border-zinc-700 pt-4">
                  ...
                  {detail.documentUrl.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                    ...
                    <a href={detail.documentUrl} target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-[#192C67] dark:text-white/70 hover:underline break-all">{detail.documentUrl}</a>
                  )}
                </div>
              )}

...
function JobFeedCard({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6 hover:border-[#192C67] dark:hover:border-[#5b8bc7] transition-all hover:shadow-md group">
      {/* Company row */}
      <div className="flex items-start justify-between gap-3 mb-4">
        <Link href={`/companies/${job.company.id}`} className="flex items-center gap-3 min-w-0">
          {job.company.logoUrl ? (
            ...
            <div className="w-12 h-12 rounded-xl bg-[#192C67] text-white text-sm font-black flex items-center justify-center flex-shrink-0">
              {job.company.name.slice(0, 2).toUpperCase()}
            </div>
          )}
      ...
      <Link href={`/jobs/${job.id}`}>
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-3 group-hover:text-[#192C67] dark:group-hover:text-[#5b8bc7] transition-colors line-clamp-2">
          {job.title}
        </h3>
      </Link>
      ...
      {skills.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-4">
          {skills.slice(0, 5).map((skill) => (
            <span
              key={skill.id}
              className="px-2 py-0.5 rounded-full text-[11px] font-semibold bg-[#192C67]/10 text-[#192C67] dark:bg-[#192C67]/20 dark:text-[#5b8bc7]"
            >
              {skill.name}
            </span>
          ))}
      ...
      {job.matchScore !== undefined && job.matchScore !== null && (
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-[11px] font-bold text-[#192C67] dark:text-[#5b8bc7]">{job.matchScore}%</span>
          </div>
          <div className="h-1.5 w-full bg-gray-200 dark:bg-gray-700 rounded-full overflow-hidden">
            <div
              className="h-full bg-[#192C67] dark:bg-[#5b8bc7] rounded-full transition-all"
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
        <button
          onClick={() => onApply(job)}
          className="flex-1 py-2.5 rounded-xl bg-[#192C67] text-white text-sm font-semibold hover:bg-[#14234f] transition-colors"
        >
          Apply Now
        </button>
        <button
